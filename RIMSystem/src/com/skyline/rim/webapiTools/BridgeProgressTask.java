...
public class BridgeProgressTask extends objProgressTask {
    /***
     * 服务器进度信息获取类
     ...
     * @param dtime 日期
     */
    public BridgeProgressTask(int type, int layercount, List<List> featureAttributes, String name, String dtime) {
    ...
    public void getObjProgressInfo(Handler handlerProgress) {
        ...
        String urlString = "http://192.168.4.4:83/api/rim/Progress/SgQiaoPlan";
        //构建查询条件
            ...
            try {
                ...
                for (int i = 0; i < featureAttributes.size(); i++) {
                    ...
                    for (int j = 0; j < featureAttributes.get(i).size(); j++) {
                        ...
                        stringBuilder.append("{\"qType\":\"" + hObj.get("qType") + "\",\"index1\":\"" + hObj.get("index1") + "\",\"index2\":\"" + hObj.get("index2") + "\",\"index3\":\"" + hObj.get("index3") + "\",\"pos\":\"" + hObj.get("pos") + "\",\"qLength\":\"" + hObj.get("qLength") + "\"},");
                    }
